...
const onLocationMessage = () => async (ctx: IMessagineContext) => {
  await ctx.mixpanel.track(`${eventTypeEnum.message}.${messageTypeEnum.location}`);
  ...
  return ctx.tg.sendLocation(opponentChatId, messageLocation.latitude, messageLocation.longitude);
};
