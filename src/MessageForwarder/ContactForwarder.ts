import TelegramBot from 'node-telegram-bot-api';
import { MessageForwarderBase } from './MessageForwarderBase';
import { DataHandler } from '../DataHandler';
import { TelegramHandler } from '../TelegramHandler';

export class ContactForwarder extends MessageForwarderBase {
  constructor(dataHandler: DataHandler, telegramHandler: TelegramHandler, chatId: number, msgContact: TelegramBot.Contact) {
    super(dataHandler, telegramHandler, chatId);
    this.msgContact = msgContact;
  }

  private msgContact: TelegramBot.Contact;

  async _forward(opponentChatId: number) {
    await this.telegramHandler.sendContact(opponentChatId, this.msgContact.phone_number, this.msgContact.first_name, this.msgContact.last_name, this.msgContact.vcard);
  }
}
